...
- **2026-09-17** — Task 26: Current time indicator in daily view. Current hour row highlighted with accent background, red line with dot at exact minute position. Time label turns accent color.
- **2026-09-17** — Task 27: Fixed prev/next navigation. Weekly view moves by week, daily by day, monthly by month. Header shows date range for weekly, full date for daily.
- **2026-09-17** — Task 28: Fixed "No email set" in Settings. Added `GET /api/auth/me` endpoint. AuthContext now fetches user profile on reload instead of just storing `{ token }`.
...
- **2026-09-17** — Task 27: Login error messages. Changed "Invalid credentials" to "Account not found. Please sign up." when email doesn't exist. Keeps "Invalid credentials" for wrong password.
